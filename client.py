...
def configura_connessione(msg):
    print(msg)
    packet = msg.encode()
    udp_header = struct.pack('!IIII', 1, port, len(
        packet), ut.checksum_calculator(packet))
...
def invia_file(fileName):
    ...
    while True:
        ...
        packet = chunk
        udp_header = struct.pack('!IIII', 2, count, len(
            packet), ut.checksum_calculator(packet))
        sock.sendto(udp_header + packet, server_address)
        count += 1
        if count == tot_packs:
            ...
            break
    file.close()
    ACCEPT_THREAD = Thread(target=termina_invio, args=(
        'Termina invio messaggio...', tot_packs,))
    ACCEPT_THREAD.start()
    ACCEPT_THREAD.join()
...
def termina_invio(msg, tot_packs):
    packet = msg.encode()
    udp_header = struct.pack('!IIII', 3, tot_packs, len(
        packet), ut.checksum_calculator(packet))
    sock.sendto(udp_header + packet, server_address)
